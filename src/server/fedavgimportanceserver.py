...
class FedavgimportanceServer(BaseServer):
    def __init__(self, args, writer, server_dataset, client_datasets, model):
        super(FedavgimportanceServer, self).__init__()
        self.args = args
        self.writer = writer
    ...
    def _calculate_data_importance(self, ids):
        data_importance_dict = {}
        for identifier in ids:
            if self.clients[identifier].model is None:
                self.clients[identifier].download(self.global_model)
            data_importance_dict[identifier] = self.clients[identifier].get_data_importance()
        return data_importance_dict
    ...
    def update(self):
        ...
        data_importance_dict = self._calculate_data_importance(selected_ids)
